...
long matrix_task_time = 0;
long comm_task_time = 0;
long comm_task_priority = 0;
unsigned long total_program_time = 0;
boolean matrix_task_running = FALSE;
boolean comm_task_running = FALSE;
...
static void matrix_task()
{
	...
	while (1) {
		...
		*/
		...
		printf("TOTAL-TIME %i: \tMatrix task finished\n", total_program_time);
		printf("TOTAL-TIME %i: \tMATRIX TASK TIME: %i\n", total_program_time, matrix_task_time);
		matrix_task_running = FALSE; // Stop counter
	}
}
...
{
	while (1) {
		vTaskSuspend(priority_handle); // Suspend priority_set_task and prevent priority changes during execution

		comm_task_running = TRUE;				// Initiate counter
		comm_task_time = 0;
		...
		fflush(stdout);
		vTaskDelay(100);
		printf("TOTAL-TIME %i: \tCOMUNICATION TASK TIME: %i\n", total_program_time, comm_task_time);
		comm_task_running = FALSE;				// Initiate counter

		vTaskResume(priority_handle); // Resume priority_set_task for checking the execution time
	}
}

static void priority_set_task(void) {

	while (1) {
		if (comm_task_time < 200) {
			if (comm_task_priority != 2) {
				comm_task_priority = 2;
				vTaskPrioritySet(communication_handle, comm_task_priority);
				printf("TOTAL-TIME %i:\tPriority set to %i\n", total_program_time, comm_task_priority);
			}
		}

		if (comm_task_time > 1000) {
...
int main(void)
{
	...
	xTaskCreate((pdTASK_CODE)priority_set_task, (signed char *)"Priority", configMINIMAL_STACK_SIZE, NULL, 5, &priority_handle);
...
void vApplicationTickHook(void)
{
	total_program_time++;			// Increase application time

	if (matrix_task_running)		// Increase matrix time if it is running
		matrix_task_time++;

	if (comm_task_running)		// Increase communication time if it is running
		comm_task_time++;
